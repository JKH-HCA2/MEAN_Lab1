...
function getFilter() {
    ...
    if (selected == "All") {
        buildTableFrame();
        $.getJSON("http://localhost:8081/api/teams", teams => {
    ...
    if (selected == "League") {
        ...
        $.getJSON("http://localhost:8081/api/leagues", leagues => {
...
function getDataByLeague() {
    ...
    $("#dataTable").remove();
    buildTableFrame();
    $.getJSON("http://localhost:8081/api/teams/byleague/" + selected, teams => {
        $.each(teams, (index, team) => {
            getTableBody(team);
        });
        addRegBtn();
...
function getDataByTeam() {
    ...
    $("#dataTable").remove();
    buildTableFrame();
    $.getJSON("http://localhost:8081/api/teams/" + selected, team => {
        getTableBody(team);
    });
    addRegBtn();
}
...
function getTableBody(team) {
    let str = `<tr>
                        ...
                        <td class='align-middle'><a role='button' class='btn btn-outline-primary' href='details.php?teamid=${team.TeamId}' >Details</a>
                        </td>                        
                    </tr>`;
