...
$(function () {
    ...
    $.getJSON("http://localhost:8081/api/teams/" + teamId, function (data) {
        ...
        if (len != 0) {
            for (let i = 0; i < len; i++) {
                let str = `<tr><td class="align-middle">${objs.Members[i].MemberName}</td><td class="align-middle">${objs.Members[i].Email}</td><td><a role="button" href="playerdetails.php?teamid=${objs.TeamId}&membid=${objs.Members[i].MemberId}" class="btn btn-outline-danger">
                Edit
              </a></td></tr>`
    ...
    let regBtn = "<a class='btn btn-primary mr-1 mt-4' id='registerBtn' href=register.php?teamid=" + teamId + ">Register</a>"
    ...
    $("#editBtn").on("click", function () {
        ...
        $("#saveChanges").on("click", function () {
            ...
            $.ajax({
                ...
                method: 'PUT',
                success: function () {
                    //document.location.href = "courses.php?instr=" + $.urlParam('instr');
                }
            })
